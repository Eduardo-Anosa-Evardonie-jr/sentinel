...
class PACGlobalDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to somepacglobald methods
    ...
    @classmethod
    def from_pacglobal_conf(self, PACGlobal_dot_conf):
        from pacglobal_config import PACGlobalConfig
        config_text = PACGlobalConfig.slurp_config_file(PACGlobal_dot_conf)
        creds = PACGlobalConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_current_masternode_vin(self):
        from pacgloballib import parse_masternode_status_vin
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = pacgloballib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = pacgloballib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = pacgloballib.blocks_to_seconds(diff)
